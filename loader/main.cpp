...
bool inject_dll_to_process(const char *dll_to_inject, HANDLE process, HANDLE main_thread)
{
    ...
    {
        printf("Couldn't allocate DLL name string in remote process: %d", GetLastError());
        return false;
    }

    if (WriteProcessMemory(process, remote_dll_name, dll_to_inject, strlen(dll_to_inject) + 1, NULL) == 0)
    {
        printf("Couldn't write DLL name string to remote process: %d", GetLastError());
        return false;
    };
    ...
    {
        printf("Couldn't create thread in remote process: %d", GetLastError());
        return false;
    }
    ...
    if (FAILED(hr))
    {
        printf("Couldn't set remote thread description: %d", hr);
        return false;
    }

    if (ResumeThread(remote_thread) == -1)
    {
        printf("Couldn't resume remote thread: %d", GetLastError());
        return false;
    };
